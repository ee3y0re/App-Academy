...
def ford_films
  ...
  # after joining, need to specify which class column belongs in (i.e. where actors.name)
  # careful on how you name column name
  execute(<<-SQL)
    SELECT
      title
    FROM
...
def films_by_cast_size
  ...
  # The following shows a table with
  # id  |             title              |  yr  | score | votes | director_id | movie_id | actor_id | ord |  id  |         name
  #which is why it needs to be grouped by title
    # SELECT
    #   *
    # FROM
    #   movies
  ...
  execute(<<-SQL)
    SELECT
      title, COUNT(castings) AS amt_actors_movie
    FROM
      movies
    JOIN
      castings ON castings.movie_id = movies.id
